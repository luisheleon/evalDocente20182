...
@section('contenido')
    ...
    <br><br>
    {!! Form::open(['route' => 'pregunta.preguntaView', 'method' => 'POST',  'class' => 'form-horizontal','id' => 'form']) !!}
    <div class="container table-responsive" style="width:80%">
        <table class="table table-striped table-bordered " id="tableFront" align="center">
            ...
            @foreach($pregunta as $pre)
                <tr>
                    <td align="center">
                        <div class="i-checks" style="width:10px; height:10px;"><input type="radio" name="id" id="id" value="{{ $pre->id }}">
                        </div>
                    </td>
                    <td>{{ $pre->pregunta  }}</td>
                    <td>@if($pre->estado == 1) Activo @else Deshabilitado @endif</td>
                </tr>
            @endforeach
